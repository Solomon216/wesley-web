...
export default function EventCard({ image, title, date, description }: EventCardProps) {
  ...
  return (
    <GlassCard
      className="overflow-hidden border border-purple-200/50 hover:border-amber-400/60 dark:border-white/10 dark:hover:border-amber-400/40 hover:-translate-y-2 hover:shadow-[0_20px_50px_rgba(107,33,168,0.12)] min-w-[280px] sm:min-w-[320px] bg-white dark:bg-card flex flex-col justify-between"
    ...
    >
      <div>
        ...
        <div className="p-6">
          ...
          >
            <p className="text-sm text-foreground mb-4 leading-relaxed" data-testid={`text-event-description-${title.toLowerCase().replace(/\s+/g, '-')}`}>
              {description}
            </p>
            <Button
              size="sm"
